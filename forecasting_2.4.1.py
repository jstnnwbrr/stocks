...
def get_data(stock_name, end_date):
    ...
    try:
        ...
        df = df[['date', 'adjOpen', 'adjHigh', 'adjLow', 'adjClose', 'adjVolume']].rename(columns={'date' : 'Date',
                                                                                    'adjOpen': 'Open',
                                                                                    'adjHigh': 'High',
                                                                                    'adjLow': 'Low',
                                                                                    'adjClose': 'Close',
...
def finalize_forecast_and_metrics(stock_name, rolling_predictions, df, n_periods, writer):  
    ...
    if direction == 'up':
        if ((target_sell_price / max_buy_price) - 1) > 0.03:
            if predicted_volatility_15_days <= 0.10: 
                recommendation = 'buy'    
            else:
                recommendation = 'hold'
        else:
    ...
    print(f"\n\n{stock_name} - Predicted High Next 15 Days: {predicted_high_15_days:.2f}",
          f"\nPredicted Low Next 15 Days: {predicted_low_15_days:.2f}",
          f"\nPredicted Avg Price Next 15 Days: {predicted_avg_15_days:.2f}",
          f"\nPredicted Volatility Next 15 Days (Coefficient of Variation): {predicted_volatility_15_days:.2f}",
    ...
    summary_df = pd.DataFrame({
        ...
        'Predicted_High_15_Days': [predicted_high_15_days],
        'Predicted_Low_15_Days': [predicted_low_15_days],
        'Predicted_Avg_15_Days': [predicted_avg_15_days],
        'Predicted_Volatility_%': [predicted_volatility_15_days],
