...
def get_data(stock_name, end_date):
    ...
    for attempt in range(3):
        try:
            ...
            df = yf.download(stock_name, start='2008-01-01', end=end_date, multi_level_index=False, progress=False)
    ...
    # Fallback to Tiingo
    try:
        print(f"[{stock_name}] Falling back to Tiingo to source data...")
        ...
        print(f"[{stock_name}] Tiingo failed: {e}")
...
def finalize_forecast_and_metrics(stock_name, rolling_predictions, df, n_periods, writer):  
    ...
    max_buy_price = round((predicted_avg_30_days * (1 - (0.5 * predicted_volatility_30_days))), 2)
    target_sell_price = round((predicted_avg_30_days * (1 + (0.5 * predicted_volatility_30_days))), 2)
    
    if direction == 'up':
        if ((target_sell_price / max_buy_price) - 1) > 0.03:
            if predicted_volatility_30_days <= 0.10: 
                recommendation = 'buy'    
            else:
                recommendation = 'hold'
        else:
...
writer = pd.ExcelWriter(f"C:\\Users\\sar81\\stocks\\stock_forecasts_{today}_1.xlsx", engine='xlsxwriter')
