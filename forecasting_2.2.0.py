...
# Set Optuna logger to only show warnings or errors
...
def get_data(stock_name, end_date):
    ...
    try:
        print(f"[{stock_name}] Sourcing data from Tiingo...")

        throttle_request()  # Check if we need to throttle before the request
        ...
        if response.status_code != 200:
            raise Exception(f"Tiingo error: {response.text}")
        ...
        df = df[['date', 'open', 'high', 'low', 'close', 'volume']].rename(columns={'date' : 'Date',
                                                                                    'open': 'Open',
                                                                                    'high': 'High',
                                                                                    'low': 'Low',
                                                                                    'close': 'Close',
        ...
        # Try to source from yfinance as backup
        for attempt in range(3):
            try:
                print(f"[{stock_name}] Attempting to source data from yfinance...")
                
                df = yf.download(stock_name, start='2015-01-01', end=end_date, multi_level_index=False, progress=False)
                ...
                print(f"[{stock_name}] Attempt {attempt + 1} with yfinance failed: {e}")
                sleep_time = 2 + random.uniform(0, 2)
                print(f"[{stock_name}] Sleeping {sleep_time:.2f}s before retrying...")
                time.sleep(sleep_time)
        
    raise Exception(f"[{stock_name}] All data sources failed.")
...
def train_models(x_train, y_train, x_test, y_test, stock_name):
    ...
    for model_name, objective in zip(studies.keys(), [objective_elastic_net,
        ...
        if best_model_for_type is not None:
            ...
            print(pd.DataFrame({'Actual' : y_test, 'Predicted' : y_pred}).tail(5))
...
def autofit_columns(df, worksheet):
    for i, column in enumerate(df.columns):
        try:
            ...
            column_width = len(column) + 4  # fallback in case of error
...
def finalize_forecast_and_metrics(stock_name, rolling_predictions, df, n_periods, writer):  
    ...
    horizon_df = rolling_forecast_df.head(10)

    predicted_high_10_days = round(horizon_df['Predicted_Close'].max(), 2)
    predicted_low_10_days = round(horizon_df['Predicted_Close'].min(), 2)
    predicted_avg_10_days = round(horizon_df['Predicted_Close'].mean(), 2)
    predicted_volatility_10_days = round(horizon_df['Predicted_Close'].std() / predicted_avg_10_days, 3)
    
    if horizon_df['Predicted_Close'].tail(3).mean() > horizon_df['Predicted_Close'].head(3).mean():
        direction = 'up'
    elif horizon_df['Predicted_Close'].tail(3).mean() < horizon_df['Predicted_Close'].head(3).mean():
        direction = 'down'
    else:
        direction = 'flat'
    
    max_buy_price = round((predicted_avg_10_days * (1 - (0.5 * predicted_volatility_10_days))), 2)
    target_sell_price = round((predicted_avg_10_days * (1 + (0.5 * predicted_volatility_10_days))), 2)
    
    if direction == 'up':
        if ((target_sell_price / max_buy_price) - 1) > 0.03:
            if predicted_volatility_10_days <= 0.10: 
                recommendation = 'buy'    
            else:
                recommendation = 'hold'
        else:
    ...
    print(f"\n\n{stock_name} - Predicted High Next 10 Days: {predicted_high_10_days:.2f}",
          f"\nPredicted Low Next 10 Days: {predicted_low_10_days:.2f}",
          f"\nPredicted Avg Price Next 10 Days: {predicted_avg_10_days:.2f}",
          f"\nPredicted Volatility Next 10 Days (Coefficient of Variation): {predicted_volatility_10_days:.2f}",
    ...
    summary_df = pd.DataFrame({
        ...
        'Predicted_High_10_Days': [predicted_high_10_days],
        'Predicted_Low_10_Days': [predicted_low_10_days],
        'Predicted_Avg_10_Days': [predicted_avg_10_days],
        'Predicted_Volatility_%': [predicted_volatility_10_days],
...
max_trials = 20 # Maximum number of trials per modeling type
patience = 10 # If performance doesn't improve within this many tests, move on
...
# Ensure the script saves data even if an error occurs
try:
    ...
    for stock_name in stock_list:
        ...
        try: 
            ...
            # Identify best model
            print(f"\n{stock_name} - Best Model:\n\n", best_model, "\n\n", f"{stock_name} - Best MAE:", best_mae_score) 
    ...
    # Save the combined summary to Excel outside the loop (in case of errors)
    ...
    # Ensure Excel file is saved even if an error occurs
    writer.close()
    print("\nExcel file saved.")
